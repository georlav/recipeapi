...
func TestMain(m *testing.M) {
	...
	rt := database.NewRecipeTable(db)

	// Import data
	for i := range data.Recipes {
		if _, err := rt.Insert(data.Recipes[i]); err != nil {
	...
	if _, err := db.Exec(`TRUNCATE TABLE recipe`); err != nil {
		log.Fatal(err)
	}
	if _, err := db.Exec(`TRUNCATE TABLE ingredient`); err != nil {
		log.Fatal(err)
	}
